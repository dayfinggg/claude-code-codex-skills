---
name: powershell
description: Production PowerShell guidance for running commands and writing, reviewing, debugging, or validating PowerShell code. Use when working with PowerShell commands, Windows shell automation, .ps1 scripts, .psm1/.psd1 modules, advanced functions, filesystem operations, quoting and path handling, pipelines and objects, error handling, remoting, encoding, Pester tests, or PSScriptAnalyzer.
---
...
- Prefer terse, outcome-first work: inspect the relevant files, make the smallest correct change, and validate the touched path.
...
- For native executables, pass arguments as separate tokens through the call operator: `& $Exe @Args`. Inspect `$PSVersionTable` and `$PSNativeCommandArgumentPassing` for version-sensitive calls, and test the receiving `argv`. Use the stop-parsing token `--%` only for interactive Windows-only commands where PowerShell parsing is the problem; avoid it in reusable scripts.
...
- Use `Set-StrictMode -Version Latest` only in a controlled host with tested upgrade behavior and when compatible with the project. For reusable cross-version modules, select and test an explicit strictness level when compatibility is contractual; otherwise avoid introducing strict mode into an existing script without checking impact.

Minimal state-changing function pattern:
...
function Remove-ProjectCache {
    ...
    if ($rootItem.PSProvider.Name -ne 'FileSystem' -or $item.PSProvider.Name -ne 'FileSystem') {
        throw 'Refusing to remove non-filesystem provider paths.'
    }

    $rootPath = [System.IO.Path]::GetFullPath($rootItem.FullName)
    $itemPath = [System.IO.Path]::GetFullPath($item.FullName)
    $relativePath = [System.IO.Path]::GetRelativePath($rootPath, $itemPath)
    if ([System.IO.Path]::IsPathRooted($relativePath) -or $relativePath -eq '..' -or $relativePath.StartsWith('..' + [System.IO.Path]::DirectorySeparatorChar) -or $relativePath.StartsWith('..' + [System.IO.Path]::AltDirectorySeparatorChar)) {
        throw "Refusing to remove path outside root: $itemPath"
    }

    if ($PSCmdlet.ShouldProcess($itemPath, 'Remove project cache')) {
...
function Get-LargeFile {
    ...
    process {
        if ($InputObject.Length -ge $MinimumBytes) {
            $InputObject
        }
    }
}
...
- Keep command output bounded in Claude Code runs; avoid unbounded recursive listings, full event logs, and full build logs unless necessary.
...
- Finalize only after re-reading the touched files or diff and reporting exactly what validation ran.
