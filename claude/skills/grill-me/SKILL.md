...
Ask exactly one short question, then wait for the answer. In a foreground Claude Code session, use `AskUserQuestion` when structured choices help; Plan mode is not required. In a non-interactive or background context, return the question to the parent or ask one concise plain-text question only when interaction is actually possible.
