...
description: Mandatory architecture pass for every software development request. Invoke before substantive action together with software-engineering, including routine code changes, bug fixes, configuration, tests, builds, deployment, review, and refactoring.
when_to_use: Use for every software task, with a lightweight pass for routine local changes and a deeper pass for boundaries, data ownership, public contracts, deployment, consistency, concurrency, security, operability, performance, migration, or failure behavior.
---
