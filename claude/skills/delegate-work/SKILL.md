---
name: delegate-work
description: "Delegate bounded work to Claude Code subagents and coordinate their results with explicit ownership, evidence, and stopping conditions. Use when the user explicitly asks for agents, subagents, delegation, parallel work, or an agent team, or when an applicable repository instruction explicitly requires delegation. Do not use merely because a task is large; keep tightly coupled, sequential, or overlapping work in the main session."
---
...
Delegate only after a direct user request or an applicable instruction explicitly authorizes it. Otherwise continue with the main session even when parallel work appears possible.
...
Keep work in the main session when it is trivial, sequential, decision-heavy, tightly coupled, likely to touch the same files, or cheaper to complete than to specify and integrate. More agents are not evidence of a better process.
...
Do not delegate an unresolved decision as if it were an implementation task. Do not let a child expand scope, contact external parties, publish, deploy, spend money, or perform destructive work without the same explicit authority the main session would need.
...
5. **Dynamic decomposition:** the main session adds bounded follow-up lanes only when initial evidence reveals them.
...
9. **Output:** the concise structure the main session needs for integration.
...
The main session remains accountable for omissions, conflicts, and unsupported claims.
...
Finish only when every required lane has a bounded owner, all required results have been integrated or consciously rejected, shared-workspace conflicts are resolved, and the main session has independently verified the combined outcome.
