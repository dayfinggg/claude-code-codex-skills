---
name: javascript
description: Engineer, review, debug, secure, or test JavaScript in .js, .mjs, .cjs, or .jsx files, browser code, libraries, scripts, and build tooling. Prefer the TypeScript skill for .ts or .tsx.
---

# JavaScript

Use this skill to make production-quality JavaScript changes. Keep the work evidence-led, scoped, and validated against the project that is actually on disk.

## Operating Model

Inspect the project before deciding:
   - Read the nearest `AGENTS.md`, `package.json`, lockfile, runtime config, framework config, test config, and relevant source files.
...
- Verify normative language status and actual deployment compatibility separately. Inclusion in ECMA-262 or a completed TC39 proposal does not prove support in the target browsers, runtime, transpiler, type libraries, or polyfill policy; use [standards, runtime, and async behavior](references/standards-runtime-async.md).
...
- Use `Promise.all` for reject-first independent work and `Promise.allSettled` when partial results are expected. `Promise.all` does not cancel sibling operations; propagate cancellation and clean up remaining work when failure should stop it.
...
1. Run formatting or linting if the repo provides a focused command.
2. Run type checking when typed JavaScript is affected or the project provides a relevant check.
3. Run relevant unit, component, or integration tests.
4. Run a build when bundler config, package exports, SSR, public APIs, or deployment-sensitive code changes.
5. For UI changes, verify the rendered result with the appropriate browser or screenshot workflow when available.
6. For security-sensitive changes, test rejected inputs and failure paths, not only the happy path.

## References

- Read [standards, runtime, and async behavior](references/standards-runtime-async.md) before using newer syntax or APIs, changing module boundaries, composing cancellation, handling streams, or targeting multiple hosts.
- Read [authoritative JavaScript sources](references/sources.md) for normative language and web-platform behavior, proposal status, and compatibility evidence. Recheck the exact runtime and browser matrix for version-sensitive work.
